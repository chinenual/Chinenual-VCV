...
struct MidiCollector : dsp::MidiGenerator<PORT_MAX_CHANNELS> {
  ...
  MidiCollector(smf::MidiFile &midiFile, int track, int &tick)
      : midiFile(midiFile), track(track), tick(tick) {}
...
struct MIDIClock {
  ...
  void incrementTick(float sampleTime, bool tempoChanged) {
    ...
    double incremental_tick = (total_time_s - time_at_last_tempo_change) * bpm /
                              SEC_PER_MINUTE * MIDI_FILE_PPQ;
...
struct MIDIRecorder : MIDIRecorderBase {
  ...
  void dataFromJson(json_t *rootJ) override {
    ...
    if (pathJ) setPath(json_string_value(pathJ));

    json_t *increment_pathJ = json_object_get(rootJ, "increment_path");
    if (increment_pathJ) increment_path = json_boolean_value(increment_pathJ);

    json_t *align_to_first_noteJ =
        json_object_get(rootJ, "align_to_first_note");
  ...
  void processMidiTrack(const ProcessArgs &args, const int track,
    ...
    if (rateLimiterTriggered) {
      ...
      if (inputs[PW_INPUT].isConnected()) {
        int pw = (int)std::round((inputs[PW_INPUT].getVoltage() + 5.f) / 10.f *
                                 0x4000);
    ...
    for (int c = 0; c < inputs[PITCH_INPUT].getChannels(); c++) {
      if (inputs[VEL_INPUT].isConnected()) {
        int vel = (int)std::round(
            inputs[VEL_INPUT].getNormalPolyVoltage(10.f * 100 / 127, c) / 10.f *
            127);
        vel = clamp(vel, 0, 127);
      ...
      if (inputs[AFT_INPUT].isConnected()) {
        int aft =
            (int)std::round(inputs[AFT_INPUT].getPolyVoltage(c) / 10.f * 127);
        aft = clamp(aft, 0, 127);
  ...
  void stopRecording(const ProcessArgs &args) {
    ...
    if (increment_path) {
      ...
      for (int i = 0; i <= 999; i++) {
        ...
        if (i > 0) newPath += string::f("-%03d", i);
        newPath += "." + extension;
        // Skip if file exists
        if (!system::isFile(newPath)) break;
      }
    }
...
static void selectPath(Module *m) {
  ...
  char *path =
      osdialog_file(OSDIALOG_SAVE, dir.c_str(), filename.c_str(), filters);
  if (path) {
    module->setPath(path);
    free(path);
  }
...
struct MIDIRecorderWidget : ModuleWidget {
  MIDIRecorderWidget(MIDIRecorder *module) {
    ...
    bpmDisplay->box.pos =
        mm2px(Vec(FIRST_X - SPACING / 2, FIRST_Y + 5 * SPACING));
    addChild(bpmDisplay);
  }
